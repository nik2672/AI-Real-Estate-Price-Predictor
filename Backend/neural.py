...
#  [I changed this to exclude the optimiser] train and save the model weights, model will also train over 200 epohs, with each batchsize of 32
def neural_model_train_and_save():
    print("Training the model...")
    model.fit(
        scaler_X.transform(X), 
        scaler_y.transform(y.values.reshape(-1, 1)), 
        epochs=200, 
        batch_size=32, 
        validation_split=0.2
    )
    model.save("neural_model.h5", include_optimizer=False)  # ✅ Save without optimizer
    print("Neural model has been trained and weights saved")
